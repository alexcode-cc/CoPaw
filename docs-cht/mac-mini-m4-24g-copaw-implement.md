...
> 本指南針對 **Mac Mini M4 (24GB)** 撰寫。24GB 統一記憶體可舒適運行 14B ~ 32B 本地模型，支援並行推理、本地 Embedding 長期記憶、MLX 原生推理，並可作為遠端 AI 引擎伺服器供其他機器使用。
>
> 若你擁有 Mac Mini M4 **16GB**，請參考 [16GB 版指南](./mac-mini-m4-copaw-implement.md)。
...
Mac Mini M4 24GB 的統一記憶體架構讓 CPU 與 GPU 共享高頻寬記憶體，搭配 Metal GPU 加速，是運行 14B ~ 32B 本地 LLM 的理想選擇。
...
與 [16GB 版指南 — 方案 B](./mac-mini-m4-copaw-implement.md#方案-bclaude-apianthropic) 設定完全相同。Claude API 不佔本地記憶體，可與本地模型並存。
...
# 檢查記憶體壓力等級（NORMAL / WARN / CRITICAL）
memory_pressure
...
| 指標 | 健康值 | 警告值 | 說明 |
|------|--------|--------|------|
| 記憶體壓力 | NORMAL | WARN / CRITICAL | CRITICAL 時系統會大量 Swap |
| Swap 使用量 | < 500 MB | > 2 GB | Swap 過大表示記憶體不足 |
| Page outs | 低 | 持續增長 | 頻繁 page out 影響效能 |
...
適合：Mac Mini 完全作為 AI 伺服器使用（包括作為遠端引擎）
...
確認 Ollama 正在使用 Metal GPU 加速：

```bash
# 查看 Ollama 日誌確認 Metal 加速
cat /tmp/ollama.log | grep -i metal
# 應看到類似 "metal" 或 "gpu" 相關訊息
...
### 4.5 Ollama Context 長度與 KV Cache 優化

較大的 Context 長度會顯著增加記憶體使用量（KV Cache）。24GB 可以使用較大的 Context，但需注意平衡：

```bash
# 建立自訂 Modelfile 調整 Context 長度
cat > ~/Modelfile-qwen3-14b-32k << 'EOF'
FROM qwen3:14b
PARAMETER num_ctx 32768
EOF

ollama create qwen3:14b-32k -f ~/Modelfile-qwen3-14b-32k
```

**Context 長度與記憶體用量參考（14B 模型）：**

| Context 長度 | 額外 KV Cache | 總記憶體（含模型） |
|-------------|--------------|------------------|
| 4096（預設） | ~0.5 GB | ~9 GB |
| 8192 | ~1 GB | ~9.5 GB |
| 16384 | ~2 GB | ~10.5 GB |
...
> **建議**：24GB 搭配 14B 模型，使用 32K Context 是舒適的選擇。32B 模型建議保持預設 4K Context。
...
與 [16GB 版指南 — 第五部分](./mac-mini-m4-copaw-implement.md#第五部分設定-discord-機器人)完全相同。
...
與 [16GB 版指南 — 第六部分](./mac-mini-m4-copaw-implement.md#第六部分設定-gmail-自動收發)完全相同。
...
與 [16GB 版指南 — 第七部分](./mac-mini-m4-copaw-implement.md#第七部分設定-github-專案自動上傳)完全相同。
...
與 [16GB 版指南 — 第八部分](./mac-mini-m4-copaw-implement.md#第八部分設定自動化工作流程)相同的四個工作流程。
...
請參考 [16GB 版指南 — 第八部分](./mac-mini-m4-copaw-implement.md#第八部分設定自動化工作流程)。
...
> 若你同時擁有 Mac Mini M4 16GB，可將本機 24GB 作為 Ollama 伺服器，讓 16GB 機器的 CoPaw 透過網路呼叫本機的大模型（14B ~ 32B），完全不佔遠端機器的記憶體。
...
**方式一：自訂 LaunchAgent（推薦，不受 brew upgrade 影響）**
...
> **注意**：`brew upgrade ollama` 會覆蓋此 plist，需重新修改。推薦使用方式一。
...
或在「系統設定 → 網路 → 防火牆 → 選項」中允許 ollama 接受傳入連線。
...
在 16GB 機器的 CoPaw 上設定遠端 Provider：
...
> **完整遠端客戶端設定**請參考 [16GB 版指南 — 方案 C](./mac-mini-m4-copaw-implement.md#方案-c遠端-ollama連接-24gb-機器)。
...
| 情境 | 建議 |
|------|------|
| **同一區域網路（LAN）** | 使用區網 IP，延遲低、設定簡單 |
| **不同網段 / VPN** | 確保 11434 埠可達，或透過 SSH 隧道 |
| **暴露於網際網路** | ⚠ 不建議。Ollama 無內建認證，若必須暴露，請使用反向代理 + 認證 |
| **固定 IP** | 建議在路由器設定 DHCP 保留，避免 IP 變動 |
...
# CoPaw 的 base_url 設為 http://localhost:11434/v1
```

### 遠端伺服器效能調校

作為遠端伺服器時，建議使用以下設定以最佳化多客戶端服務：
...
```bash
cat > ~/check-remote-server.sh << 'SCRIPT'
#!/bin/bash
echo "=== Ollama 遠端伺服器狀態 ==="
echo ""
...
chmod +x ~/check-remote-server.sh
```
...
# 卸載不用的模型
curl http://localhost:11434/api/generate -d '{"model":"不用的模型","keep_alive":0}'
```

### Q: 遠端客戶端無法連線本機 Ollama

**檢查清單**：
1. 本機 Ollama 是否運行：`curl http://localhost:11434/api/tags`
2. `OLLAMA_HOST` 是否設為 `0.0.0.0:11434`（非 `127.0.0.1`）
3. 防火牆是否放行 11434 埠
4. 從遠端測試：`curl http://本機IP:11434/api/tags`
5. base_url 格式需含 `/v1`：`http://192.168.1.100:11434/v1`
...
`~/.config/himalaya/config.toml`、`~/.copaw/HEARTBEAT.md` 與 [16GB 版指南 — 完整設定檔參考](./mac-mini-m4-copaw-implement.md#完整設定檔參考)相同。
